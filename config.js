...
const { isvip } = require('./lib/lang/pt')
...
const rename = JSON.parse(fs.readFileSync('./lib/config/Gerais/rename.json'))
const verify = JSON.parse(fs.readFileSync('./lib/config/Gerais/verify.json'))
const isVEr = verify.includes(verify)
const vips = JSON.parse(fs.readFileSync('./lib/config/Gerais/vipuser.json'))
const isVIPs = vips.includes(vips)
...
module.exports = kconfig = async (kill, message) => {
	...
	const { type,	 from, t, sender, author, isGroupMsg, chat, chatId, caption, isMedia, mimetype, quotedMsg, quotedMsgObj, mentionedJidList } = message
	...
	const chats = (type === 'chat' || type == "buttons_response") ? body : ((type === 'image' || type === 'video')) ? caption : ''
	body = (type == 'chat' && body.startsWith(prefix) || type == "buttons_response") ? body : (((type === 'image' || type === 'video') && caption) && caption.startsWith(prefix)) ? caption : ''
	...
	try {
		...
		/*if (isGroupMsg && isCmd && !isOwner && !isBot && groupId) {
			const totalMem = chat.groupMetadata.participants.length*/
		/*if (chat.groupMetadata.participants.includes(config.Owner)) {
			console.log(color('[NOVO]', 'red'), color(`Grupo ${chat.contact.name} autorizado.`, 'yellow'))
		...
		if (isGroupMsg && autoSticker && isMedia && isImage && !isCmd && !isBot || !isGroupMsg && !isCmd && isMedia && isImage) {
			const mediaData = await decryptMedia(message, uaOverride)
			return await kill.sendImageAsSticker(from, `data:${mimetype};base64,${mediaData.toString('base64')}`, { author: config.Sticker_Author, pack: config.Sticker_Pack, keepScale: true })
			.catch(() => { })
		}
		
		// Auto-sticker de videos & gifs
		if (isGroupMsg && autoSticker && isMedia && isVideo && !isCmd && !isBot || !isGroupMsg && !isCmd && isMedia && isVideo) {
			const mediaData = await decryptMedia(message, uaOverride)
			return await kill.sendMp4AsSticker(from, `data:${mimetype};base64,${mediaData.toString('base64')}`, null, { stickerMetadata: true, pack: config.Sticker_Pack, author: config.Sticker_Author, fps: 10, crop: false, loop: 0 })
			.catch(() => { })
		}
...
// BOAS VINDAS PV's
		//if (verify.includes(user)) return console.log(color('> [VERIFY]', 'red'), color(`Ignorando comando de ${pushname} - [${user.replace('@c.us', '')}] pois ele está verificado...`, 'yellow'))
		
		if (!isGroupMsg && !isMedia && !vips.includes(user) && !verify.includes(user)) {
			isMsg[user] == null ? isMsg[user] = 1 : isMsg[user] += 1
			if (isMsg[user] < 2) {
				verify.push(user)
				fs.writeFileSync('./lib/config/Gerais/verify.json', JSON.stringify(verify))
				//await kill.sendText(from, "bem vindo", id)
				//await kill.reply(from, '😍 - Olá, obrigada por estar escolhendo me usar e meu prefix caso tenha duvidas é "!"\n🛡 - Logo fica:\n\n✅ - *!menu*\nSe veio fazer sticker basta enviar a imagem que vou converter para você ‼\n\n📝 - Vale lembrar antes de usar esse bot é *necessário/IMPORTANTE* ler os termos e condições que estão descritos no *!site* ou nesse link:\n\n🔗 linktr.ee/irisbot\n\n⁉ - Quer um tutorial básico de como usar a iris? Basta dar !tutorial', id)
				await kill.sendButtons(from, '*OLÁ SEJA BEM VINDO AO MEU ESPAÇO* 🤖\n\n*SE VOCÊ DESEJA CONTRATAR O BOT FALA COM O MEU DONO*\n\n*🪀65-996684082*🪀',  [
					{	
						"id":"id1",
						"text": "🖌 Fazer Figurinha"
					},
					{
						"id":"!menu",
						"text": "🚫 Regras do Bot"
					}
				],'Mensagem de verificação', "digite !menu")
	
			} else if (verify.includes(user)) return console.log(color('> [VERIFY]', 'red'), color(`Ignorando comando de ${pushname} - [${user.replace('@c.us', '')}] pois ele está verificado...`, 'yellow'))
		...
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('🚫 Regras da Bot')) await kill.sendText(from, '❌ _Sem Flood // 2seg entre comandos_\n\n❌ _Não ligue para o bot, insta block._\n\n❌ _Não envie travas ou textos longos, block!_\n\n❌ _Não adianta enviar link de grupo._', id) } catch (error) { return } }
		
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('🖌 Fazer Figurinha')) await kill.sendText(from, 'caso esteja no pv *não precisa* ficar digitando comando na *legenda* da(o) imagem/gif/video, basta enviar a imagem/gif/video que transformarei em *sticker automático* ^-^', id) } catch (error) { return } }
		
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('💸 Valores')) await kill.sendButtons(from, '*─ 「 DIAS CORRIDOS PVs」 ─*\n✅ R$ 2 = 7 Dias\n✅ R$ 5 = 20 Dias\n✅ R$ 8 = 30 Dias\n✅ R$ 30 = PERMANENTE\n\n*── 「 30 DIAS - Grupos 」 ──*\n\n✅ R$ 6 = Até 100 membros\n✅ R$ 14 = Até 255 membros\n\n*── 「 PERMANENTE 」 ──*\n✅ R$ 35 = Até 100 membros\n✅ R$ 50 = Até 255 membros\n\nOs comandos são ilimitados em grupo!\n\nAcréscimo de R$ 5 para incluir funções VIP', [{ id: "0", "text": '📝 Instruções'}], 'Aqui estão os valores dos Vips: 💰\n', "Veja as instruções antes de contratar!")	} catch (error) { return } }
		
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('@irisbot_ofc')) await kill.reply(from, 'me segue no insta ai linds <3: \nhttps://www.instagram.com/irisbot_ofc/', id) } catch (error) { return } }
		
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('😎 Contatar dono')) await kill.reply(from, mess.everhost(), id) } catch (error) { return } }
		
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('💰 Valores')) await kill.sendButtons(from, '*── 「 30 DIAS 」 ──*\n✅ R$ 6 = Até 100 membros\n✅ R$ 14 = Até 255 membros\n\n*── 「 PERMANENTE 」 ──*\n✅ R$ 35 = Até 100 membros\n✅ R$ 50 = Até 255 membros\n\nOs comandos são ilimitados em grupo!\n\nAcréscimo de R$ 5 para incluir funções VIP', [{ id: "0", "text": '📝 Instruções'}], 'Aqui estão os valores do Grupo:💰', "Veja as instruções antes de contratar!")	} catch (error) { return } }
		
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('📝 Instruções')) await kill.sendButtons(from, '- Não haverá nenhum tipo de reembolso.\n\n- O Vip não pode ser transferido.\n\n- Não fica imune as regras.\n\n- Entende que o Bot pode ser banido.\n\n- *Se você adquiriu Vip ou Grupo permanente,*\n\n*caso o Bot seja banido, sua aquisição não será perdida!*', [{ id: "0", "text": '😎 Contatar dono'}], 'Você deve concordar com os seguintes termos:', "Dúvidas? Envia uma mensagem ao meu dono.")	} catch (error) { return } }
		
		/*if pvauto (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('✅ Verifique-se')) if (!isUser) return await kill.reply(from, mess.registro(), id)
		setTimeout(() => {
			verify.push(user)
		fs.writeFileSync('./lib/config/Gerais/verify.json', JSON.stringify(verify))
		kill.reply(from, "😍 Você acabou se verificar, acesse seus comandos em !menu", id)
		gaming.addValue(user, Number(-5), nivel, 'money');
		kill.sendText(ownerNumber[0], "*⭐[SUCESSO]⭐*\nesse número: ${nomevip} acabou de se tornar um vip temporário e ele irá durar *24 horas*");
			console.log(color('[COMPROU-VIP-TIME]', 'yellow'), color(`Nome: ${pushname}`, 'yellow'))
			}, 1000)
		setTimeout(() => {
			console.log(color('VIP TIME UP END', 'yellow'), color(`Nome: ${pushname}`, 'red'))
				kill.reply(from, "*⭐[SUCESSO]⭐*\n\n- Você acabou de virar um usuário *verificado* ✅", id)
				kill.sendText(ownerNumber[0], "*⭐[SUCESSO]⭐*\nesse número: ${nomevip} acabou de se tornar um vip temporário");
			let pos = verify.indexOf(user)
			verify.splice(pos, 1)
			fs.writeFileSync('./lib/config/Gerais/verify.json', JSON.stringify(verify))
		}, 300000000)	} catch (error) { return } }*/
		
		if (!isFiltered(from) && !isMedia && !isCmd) { try { if (chats.includes('🔗 Links & Termos')) await await kill.sendButtons(from, 'Grupo deste bot: https://chat.whatsapp.com/EmXpPzIcXYmG9uP9WzEfaq\n\nLink Principal:\nlinktr.ee/irisbot \n\nPara ler os temos desse bot clique nesse outro link:\nwww.botiris.cf/politica-de-privacidade-do-site/termos\n\nTutorial basico de como usar a iris:\nhttps://youtu.be/3ZA9WvIOpFY' ,  [
			{
						"id":"id1",
						"text": "🔸 @irisbot_ofc"
					}
				], "Links e Termos", "leia os termos")	} catch (error) { return } }
		/*try {
						
					}*/
		// Caso deseje criar siga o estilo disso abaixo, para usar a base remova a /* e a */
		/*if (!isFiltered(from) && !isCmd) { try { if (chats.toLowerCase() == '✅ Verifique-se') 'if (!isUser) return await kill.reply(from, mess.registro(), id)
		setTimeout(() => {
			verify.push(user)
		fs.writeFileSync('./lib/config/Gerais/verify.json', JSON.stringify(verify))
		kill.reply(from, "😍 Você acabou se verificar, acesse seus comandos em !menu", id)
		...
		if (isCmd && isFiltered(from) && !isGroupMsg && !vips.includes(user) && !isOwner) { 
			await gaming.addValue(user, Number(-100), nivel, 'xp'); 
			await kill.reply(from, '*⚠[Anti-Flood-PV]⚠*\nEspere 5 segundos antes de dar outro comando', id);
			return console.log(color('> [FLOOD AS]', 'red'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`"[${prefix}${command.toUpperCase()}] [${args.length}]"`, 'red'), 'DE', color(`"${pushname} - [${user.replace('@c.us', '')}]"`, 'red')) 
		}
		
		// Anti Flood para grupos
		if (isCmd && isFiltered(from) && isGroupMsg && !vips.includes(user) && !isOwner) { 
			await gaming.addValue(user, Number(-100), nivel, 'xp'); 
			await kill.reply(from, '*⚠[Anti-Flood]⚠*\nEspere 5 segundos antes de dar outro comando\nVocê perdeu -100 *[XP]*', id);
			return console.log(color('> [FLOOD AS]', 'red'), color(moment(t * 1000).format('DD/MM/YY HH:mm:ss'), 'yellow'), color(`"[${prefix}${command.toUpperCase()}] [${args.length}]"`, 'red'), 'DE', color(`"${pushname} - [${user.replace('@c.us', '')}]"`, 'red'), 'EM', color(`"${name || formattedTitle}"`)) 
		}
...
case 'ttp':
	...
	await axios.get(`https://api.xteam.xyz/ttp?file&text=${encodeURIComponent(body.slice(5))}`, { responseType: 'arraybuffer' }).then(async (response) => {
		const attp = Buffer.from(response.data, 'binary').toString('base64');
		await kill.sendImageAsSticker(from, attp, { author: config.Sticker_Author, pack: config.Sticker_Pack, keepScale: true })
	})
	break
...
case 'attp':
	...
	break
			...
			case 'nobg':
				if (!vips.includes(user)) return await kill.reply(from, 'Você não é um VIP para usar esse comando.', id)
				if (isImage || isQuotedImage) {
			...
			case 'legiao':
				...
				await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/EmXpPzIcXYmG9uP9WzEfaq', '\nEsse grupo é apenas a recepção do grupo Legião Z (gp com link redefinido), ao entrar leia a descrição, responda as perguntas e aguarde pela chegada de um administrador para te adicionar - siga os passos que ele pedir.')
				break
				
			case 'chat':
				if (isGroupMsg) return await kill.reply(from, mess.sopv(), id)
				await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/EmXpPzIcXYmG9uP9WzEfaq', '\nGrupo para chat entrae brow S2')
				break
				
			case 'grupofig':
			case 'grupoiris':
				if (isGroupMsg) return await kill.reply(from, mess.sopv(), id)
				await kill.sendLinkWithAutoPreview(from, 'https://chat.whatsapp.com/EmXpPzIcXYmG9uP9WzEfaq', '\nGrupo oficial da iris com stickers automático e info sobre a iris com bonus de outros bots!!')
				break
			...
			case 'sporn':
				if (!vips.includes(user)) return await kill.reply(from, 'Você não é um VIP para usar esse comando.', id)
				if (isGroupMsg && !isNsfw) return await kill.sendButtons(from, "ou clique em um dos botões, !nsfw on para ativar e off para desativar", [
		{
		    "id": "1",
			"text": "!nsfw on"
		},
	...
	], 'Aqui não permitimos safadezas e impurezas 😡, caso um administrador queira permitir, use !nsfw on, ou você pode usar no PV.')
			...
			case 'akinator':;case 'aki':
				try {
					if (args[0].toLowerCase() == '-r') {
						...
						if (!akinm) return await kill.sendButtons(from, mess.aki(), [
		{
		    "id": "1",
			"text": "!aki -r 0"
		},
		{
			...
			"text": "!aki -r 1"
	...
	], 'Pergunta', 'lembre de esperar 3 segundos para responder (clique no botão 0 para sim e 1 para não)')
						...
						} else { await await kill.sendButtons(from, mess.akistart(aki), [
		{
		    "id": "1",
			"text": "!aki -r 0"
		},
		{
			...
			"text": "!aki -r 1"
						...
						jogadas = 0
						await kill.sendButtons(from, mess.akistart(aki), [
		{
		    "id": "1",
			"text": "!aki -r 0"
		},
		{
			...
			"text": "!aki -r 1"
					...
					} else return await kill.sendButtons(from, mess.akistart(aki), [
		{
		    "id": "1",
			"text": "!aki -r 0"
		},
		...
		},
		{
			...
			case 'rolette':;case 'roleta':
				if (!isxp) return await kill.sendButtons(from, "ou clique em um dos botões, !rank on para ativar e off para desativar", [
					{
						"id": "1",
						"text": "!rank on"
					},
				...
				], 'Para usar um comando desse tipo, peça para seu adminitrador caso você não seja que ative o sistema de XP.')
			...
			/*case 'daily':
				const limitdaily = await gaming.getLimit(user, daily);if (gaming.isLimit(limitdaily) == 1) return await kill.reply(from, mess.limitgame(), id)
				//const checkxpr = await gaming.getValue(user, nivel, 'xp');const xpMenorT = parseInt(checkxpr / 2, 10)
				var nrolxpdaily = Math.floor(Math.random() * -milSort) - Number(args[0]);var prolxpdaily = Math.floor(Math.random() * milSort) + Number(args[0])
				await gaming.addValue(user, Number(100), nivel, 'xp');
				await kill.reply(from, "parabens voce ganhou 100 de xp", id)
				side == 1 ? await gaming.addValue(user, Number(nrolxpdaily), nivel, 'xp') : await gaming.addValue(user, Number(prolxpdaily), nivel, 'xp')
				break*/

					
			/*case 'daily':;case 'teste0':
				const limitrld = await gaming.getLimit(user, daily);if (gaming.isLimit(limitrld) == 1) return await kill.reply(from, mess.limitgame(), id)
				const checkxprd = await gaming.getValue(user, nivel, 'xp');const xpMenorT = parseInt(checkxprd / 2, 10)
				//if (isNaN(args[0]) || !isInt(args[0]) || Number(args[0]) >= xpMenorT || Number(args[0]) < 250) return await kill.reply(from, mess.gaming(checkxpr, xpMenorT), id)
				var nrolxp = Math.floor(Math.random() * milSort) + Number(args[0]);var prolxp = Math.floor(Math.random() * milSort) + Number(args[0])
				side == 1 ? await kill.sendFileFromUrl(from, 'https://i.ibb.co/vQj6nq4/roleta1.png', 'rol1.png', mess.loseshot(nrolxp), id) : await kill.sendFileFromUrl(from, 'https://i.ibb.co/PwKR2nR/roleta.jpg', 'rol.jpg', mess.winshot(prolxp), id)
				side == 1 ? await gaming.addValue(user, Number(nrolxp), nivel, 'xp') : await gaming.addValue(user, Number(prolxp), nivel, 'xp')
		   ...
		   case 'ins':
			...
			case 'nsfw':
				if (args.length !== 1) return await kill.sendButtons(from, "(clique no botão on para ativar e off para desativar)", [
		{
		    "id": "1",
			"text": "!nsfw on"
		},
	...
	], 'Especifique entre <on> ou <off>, se quiser algo mais detalhado use o comando com -help.')
				if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
					if (args[0].toLowerCase() == 'on') {
		...
		},
		{
			...
			case 'profile':;case 'perfil':
				if (profile == 'ERROR: 401' || profile == '' || profile == undefined) profile = 'https://encrypted-tbn0.gstatic.com/images?q=tbn%3AANd9GcTQcODjk7AcA4wb_9OLzoeAdpGwmkJqOYxEBA&usqp=CAU'
				if (isGroupMsg) {
				...
				try {
					...
					if (args[0].toLowerCase() == '-all') {
						for (let ids of chatall) {
							...
							if (!cvk.isReadOnly) {
								try {
									await kill.sendButtons(ids, `${msg}`, [{ id: "0", "text": '🚫 Regras da Bot'}, { id: "0", "text": '🔗 Links & Termos'}], `[Transmissão de ${pushname} ]`, "me segue no insta @irisbot_ofc")	//kill.sendText(ids, `[Transmissão de ${pushname} ]\n\n${msg}`)
			...
			case 'add':
				await kill.reply(from, "Esse comando está em manutenção", id)
				/*if (!isGroupMsg) return await kill.reply(from, mess.sogrupo(), id)
				if (!isGroupAdmins) return await kill.reply(from, mess.soademiro(), id)
				if (!isBotGroupAdmins) return await kill.reply(from, mess.botademira(), id)
		...
		},
		{
			...
			case 'pgay':
				//if (!isOwner) return await kill.reply(from, "Esse comando é VIP", id)
				if (!vips.includes(user)) return await kill.reply(from, 'Você não é um VIP para usar esse comando.', id)
				if (isGroupMsg && !isNsfw) return await kill.sendButtons(from, "ou clique em um dos botões, !nsfw on para ativar e off para desativar", [
		{
		    "id": "1",
		...
		},
		{
			...
			case 'addvip':;case 'promovervip':;case 'darvip':
				if (!isOwner) return await kill.reply(from, mess.sodono(), id)
				if (mentionedJidList.length == 0 && !quotedMsg) return await kill.reply(from, mess.semmarcar(), id)
				if (mentionedJidList.length !== 0) marcarvip = await kill.getContact(mentionedJidList[0])
				var nomevip = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? marcarvip.id : user)
				{
					vips.push(nomevip)
					await fs.writeFileSync('./lib/config/Gerais/vipuser.json', JSON.stringify(vips))
					await kill.reply(from, mess.promotevip(nomevip, vips), id)
					console.log(color('[PROMOVER-VIP]', 'yellow'), color(`Dono: ${pushname}`, 'red'))
				} 
			 	break
				...
				case 'deletaracc':
					...
					if (mentionedJidList.length !== 0) {
						var mentionUsr = await kill.getContact(mentionedJidList[0])
					}
					var xpUserGet = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? mentionUsr.id : user)
					const wtfXPP = await gaming.getValue(xpUserGet, nivel, 'xp')
					const shesMSGG = await gaming.getValue(xpUserGet, nivel, 'msg')
					const uzerlvll = await gaming.getValue(xpUserGet, nivel, 'level')
					const icoinqtdd = await gaming.getValue(xpUserGet, nivel, 'coin')
					await gaming.addValue(xpUserGet, Number(`-${wtfXPP}`), nivel, 'xp')
					await gaming.addValue(xpUserGet, Number(`-${shesMSGG}`), nivel, 'msg')
					await gaming.addValue(xpUserGet, Number(`-${uzerlvll}`), nivel, 'level')
					await gaming.addValue(xpUserGet, Number(`-${icoinqtdd}`), nivel, 'coin')
					await kill.sendText(from, 'Usuario apagado com successo!')
					break
			...
			case 'vips':
				var nomevip = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? marcarvip.id : user)
					//vips.push(nomevip)
					await fs.writeFileSync('./lib/config/Gerais/vipuser.json', JSON.stringify(vips))
				await kill.reply(from, mess.promoteviplis(nomevip, vips), id)
				break

				/*case 'vips2':
				var nomevip = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? marcarvip.id : user)
				let vipsgral = `-----[ *VIPS* ]----\n\n`
				vips.sort((a, b) => (a.xp < b.xp) ? 1 : -1)
				//try {
					for (let i = 0; i < vips.length; i++) {
						//if (groupMembersId.includes(vips[i].id)) {
							const bvipsuer = await kill.getContact(vips[i].id)
							var vipUserName = bvipsuer.pushname == null ? 'wa.me/' + nivel[i].id.replace('@c.us', '') : bRandV.pushname
							nomevip = nomevip + 1
							vipsgral += `${nomevip} → *${vipUserName}*\n\n`
			...
			case 'removervip2':
				...
				var nomevip = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? marcarvip.id : user)
				{
					vips.splice(mentionedJidList[0], 1);await fs.writeFileSync('./lib/config/Gerais/vipuser.json', JSON.stringify(vips))
					console.log(color('[REBAIXAR-VIP]', 'yellow'), color(`Dono: ${pushname}`, 'red'))
					await kill.reply(from, mess.vipend(nomevip), id)
				} 
			 	break

				 case 'verify':
					if (verify.includes(user)) return await kill.reply(from, "Você já está registrado(a)", id)
					try {
						if (!isUser) return await kill.reply(from, mess.registro(), id)
						setTimeout(() => {
							verify.push(user)
						fs.writeFileSync('./lib/config/Gerais/verify.json', JSON.stringify(verify))
						kill.reply(from, `😍 parabens ${pushname} Você acabou se verificar, acesse seus comandos em !menu`, id)
						gaming.addValue(user, Number(-5), nivel, 'money');
						kill.sendText(ownerNumber[0], `*⭐[SUCESSO]⭐*\nesse número: ${nomevip} acabou de se tornar um vip temporário e ele irá durar *24 horas*`);
							console.log(color('[COMPROU-VIP-TIME]', 'yellow'), color(`Nome: ${pushname}`, 'yellow'))
							}, 1000)
						...
						console.log(color('[TIME UP]', 'crimson'), color(`→ Obtive erros no comando ${prefix}${command} → ${error.message} - Você pode ignorar.`, 'gold'))
					}
				break
			...
			case 'coletarvip':
				try {
					if (!isUser) return await kill.reply(from, mess.registro(), id)
					var nomevip = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? menUser.id : user)
					const buyvip2 = await gaming.getValue(user, nivel, 'coin')
					if (buyvip2 <= 5000) return await kill.reply(from, mess.coinsufic7(buyvip2), id)
					if (vips.includes(user)) return await kill.reply(from, mess.isvip(), id)
					setTimeout(() => {
						vips.push(user)
					fs.writeFileSync('./lib/config/Gerais/vipuser.json', JSON.stringify(vips))
					kill.reply(from, "😍 Você acabou de virar premium, acesse seus comandos em !menuvip", id)
					gaming.addValue(user, Number(-5000), nivel, 'coin');
					kill.sendText(ownerNumber[0], "*⭐[SUCESSO]⭐*\nesse número: ${nomevip} acabou de se tornar um vip temporário e ele irá durar *24 horas*");
						console.log(color('[COMPROU-VIP-TIME]', 'yellow'), color(`Nome: ${pushname}`, 'yellow'))
						}, 1000)
					setTimeout(() => {
						console.log(color('VIP TIME UP END', 'yellow'), color(`Nome: ${pushname}`, 'red'))
							kill.reply(from, "*⭐[SUCESSO]⭐*\n\n- Você acabou de se tornar *um vip* ✅", id)
							kill.sendText(ownerNumber[0], "*⭐[SUCESSO]⭐*\nesse número: ${nomevip} acabou de se tornar um vip temporário");
						let pos = vips.indexOf(user)
						vips.splice(pos, 1)
						fs.writeFileSync('./lib/config/Gerais/vipuser.json', JSON.stringify(vips))
					}, 3600)
				} catch (error) {
...
if (mentionedJidList.length !== 0) marcarvip = await kill.getContact(mentionedJidList[0])
var nomevip = quotedMsg ? quotedMsgObj.sender.id : (mentionedJidList.length !== 0 ? marcarvip.id : user); {
  let pos = vips.indexOf(mentionedJidList[0])
  vips.splice(pos, 1)
  await fs.writeFileSync('./lib/config/Gerais/vipuser.json', JSON.stringify(vips))
  console.log(color('[REBAIXAR-VIP]', 'yellow'), color(`Dono: ${pushname}`, 'red'))
  await kill.reply(from, mess.vipend(nomevip), id)
...
break
			...
			case 'call':
				//if (!vips.includes(user)) return await kill.reply(from, 'Você não é um VIP para usar esse comando.', id)
				/*if (!isOwner) return*/await kill.reply(from, "_⚠ Esse comando foi removido por violar os termos e privacidade descritos no site iris bot_ (clica.ai/bUjrZBN)", id)
		...
		},
		{
            ...
            ],'GORDÃO BOT 🤖', "para ver minha tabela de preços digite !tabela", id)
			
			await kill.sendButtons(from, `⭐ _Seu tipo de usuário:_ *${advip}*\n` + mess.menu(pushname, time, theMsg, uzrXp, getReqXP(uzrlvl), uzrlvl, mping, patente, message.id) + mess.vipmet(), [
				{
                    "id":"id1",
                    "text": "🔗 Links & Termos"
                },
			...
			], 'Usuário vip detectado ✅', "este menu é para vips")
						break
			//break
			...
			//case 'menu11':
				...
				//const imgMenu = await kill.sendFileFromUrl(from, 'https://i.ibb.co/cC5SPKZ/iris.png', 'iris.png', id)
				//await kill.sendButtons(from, imgMenu, [{ id: "0", "text": '!menu'}], '🔸 ops chamem os mecânicos!')
				//await kill.sendButtons(from, "\n╭───「 ALGUNS FIXADOS! 」\n│\n│ 📌 - *grupo oficial de infos e noticias da iris*\n│ ᐳ 🌅 → !grupoiris\n│\n│ ᐳ 📣 → *!chat*\n│ ᐳ _Grupo oficial de Chat da IRIS._\n│\n│ ᐳ ❓ → *!faq*\n│ ᐳ _perguntas-e-duvidas-frequentes-da-iris._\n│\n╰───────────────────\n\n╭───「 🔖 Comandos 📌 」\n│\n│ ⚠️ - *Não floode senão os comandos podem cair!*\n│\n│ ⚠️ - *Se falhar, o servidor caiu temporariamente.*\n│\n│ 🏖️ - *1° → !Stickers*\n│ ᐳ _Comandos que usam stickers._\n│\n│ 🌐 - *2° → !Down*\n│ ᐳ _Comandos de Downloads._\n│\n│ 🔐 - *3° → !Dono*\n│ ᐳ _Comandos que só meu dono pode usar._\n│\n│ 🧑🏼‍💻 - *4° → !Admins*\n│ ᐳ _Comandos para administradores._\n│\n│ 🎰 - *5° → !Games*\n│ ᐳ _Mini-Jogos para jogar!_\n│\n│ 🔞 - *6° → !Adult*\n│ ᐳ _Lista de comandos +18!_\n│\n│ 🖨️ - *7° → !Maker*\n│ ᐳ _Fabrica de imagens!_\n│\n│ 🍙 - *8° → !Otaku*\n│ ᐳ _Lista de comandos de Anime!_\n│\n│ ℹ️ - *9° → !Dados*\n│ ᐳ _Comandos Informativos!_\n│\n│ 🎬 - *10° → !Midia*\n│ ᐳ _Comandos de Mídia!_\n│\n│ 🚪 - *11° → !Outros*\n│ ᐳ _Outros comandos!_\n│\n╰───────────────────", [{ id: "0", "text": '@mwythor'}], 'caso queira ver info adicional escreva !patente')
				//break
			
			case 'menu1':
			//const imgMenu = await kill.sendFileFromUrl(from, 'https://i.ibb.co/cC5SPKZ/iris.png', 'iris.png', id)
			await kill.reply(from, 'Vale lembrar antes de usar esse bot é *necessário/IMPORTANTE* ler os termos e condições que estão descritos no *!site* ou nesse link:\n\nabre.ai/termosiris\n\nQuer um tutorial básico de como usar a iris? Basta dar !tutorial', id)
			await kill.sendButtons(from, '\n╭───「 ALGUNS FIXADOS! 」\n│\n│ ᐳ 📣 → *!chat*\n│ ᐳ _Grupo oficial de Chat da IRIS._\n│\n│ ᐳ ❓ → *!site ou !faq*\n│ ᐳ _perguntas-e-duvidas-frequentes-da-iris._\n│\n╰───────────────────\n\n╭───「 🔖 Comandos 📌 」\n│\n│ ⚠️ - *Não floode senão os comandos podem cair!*\n│\n│ ⚠️ - *Se falhar, o servidor caiu temporariamente.*\n│\n│ 🏖️ - *1° → !Stickers*\n│ ᐳ _Comandos que usam stickers._\n│\n│ 🌐 - *2° → !Down*\n│ ᐳ _Comandos de Downloads._\n│\n│ 🔐 - *3° → !Dono*\n│ ᐳ _Comandos que só meu dono pode usar._\n│\n│ 🧑🏼‍💻 - *4° → !Admins*\n│ ᐳ _Comandos para administradores._\n│\n│ 🎰 - *5° → !Games*\n│ ᐳ _Mini-Jogos para jogar!_\n│\n│ 🔞 - *6° → !Adult*\n│ ᐳ _Lista de comandos +18!_\n│\n│ 🖨️ - *7° → !Maker*\n│ ᐳ _Fabrica de imagens!_\n│\n│ 🍙 - *8° → !Otaku*\n│ ᐳ _Lista de comandos de Anime!_\n│\n│ ℹ️ - *9° → !Dados*\n│ ᐳ _Comandos Informativos!_\n│\n│ 🎬 - *10° → !Midia*\n│ ᐳ _Comandos de Mídia!_\n│\n│ 🚪 - *11° → !Outros*\n│ ᐳ _Outros comandos!_\n│\n╰───────────────────',  [
                {
                    "id":"id1",
                    "text": "!site para ver tudo sobre o site"
                },
                {
                    "id":"id2",
                    "text": "menu de jogos aqui^-^"
                },
                {
                    "id":"!menu",
                    "text": "teste  "
                }
            ],'Vale lembrar antes de usar esse bot é necessário/IMPORTANTE ler os termos e condições que estão descritos no !site ou nesse link:\n\nabre.ai/termosiris\n\nQuer um tutorial básico de como usar a iris? Basta dar !tutorial', "Olá bem vindo ao menu da iris, caso queira ver sua patente, xp, lvl, etc. Escreva !patente")
			break

			case 'tabela':
			await kill.reply(from, "*🥇GORDÃO CC'S🥇*🔥\n\n*📍APRESENTA O MELHOR MATERIAL FULL DA NET!!!*\n\n*🚀 CCS FULL DADOS 🚀*\n*💳 CLASSIC- NOME + CPF   = R$ 14.00*\n*💳 STANDARD- NOME +  CPF = R$ 16.00*\n*💳 GOLD- NOME + CPF     = R$ 20,00*\n*💳 PLATINIUM- NOME + CPF = R$ 30,00*\n*💳 ELO- NOME + CPF           = R$ 35.00*\n*💳 CORPORATE- NOME + CPF = R$ 40.00*\n*💳 BUSINESS- NOME + CPF  =  R$ 45.00*\n*💳 BLACK - NOME + CPF = R$     R$ 70.00*\n*💳 INFINITE- NOME + CPF   = R$ 70.00*\n*💳 AMEX- NOME + CPF =R$       100.00*\n\n===========================\n\n*⚠️ COMPRE SE SOUBER USAR*⚠️\n\n❌ *NAO VENDO CC PARA NENHUM TIPO DE  DELIVERY*❌\n*CASO TENTE O RISCO E SEU*✅\n\n*⚠️ TODAS AS CCS SÃO TESTADA NA HORA*⚠️", id)
			break


case 'Mandarpack':
	if (!isOwner) return await kill.reply(from, mess.sodono(), id)
	await kill.sendListMessage(from, listmessa, sections)
	rows = {description: 'manda uma lista', rowId: 'id1', title: 'teste'}
	var sections = [{rows: rows, title: 'teste'}]
	var listmessa = {
		title: "teste",
		description: "testando lista",
		actionText: 'id2'
	}
	break
	...
	//case 'testekk':
		//const fotothumb = {text: 'oi', sendEphemeral: true,"externalAdReply": {"title": `NÃO CLIQUE AQUI!!! (sério)`,"body": "","previewType": "PHOTO","thumbnailUrl": "","thumbnail": fs.readFileSync('./logos/logo.jpg'),"sourceUrl": `oi https://api.whatsapp.com/send?phone=${setting.numerodono}&text=Eu%20não%20resisti%20😭😭%20me%20come%20pfvr%20`},mentionedJid:[sender]
		//await kill.reply(from, help(prefix), text, {quoted: mek, quoted: { key: 
		//	(from, help(prefix), text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "TEXTO QUE FICA EM BAIXO DA FOTO", 'jpegThumbnail': fs.readFileSync('./assets/botlogo.webp')}}}})
 //break
		//break
		
case 'tes':
await kill.sendListMessage(from, 'Title', 'subTitle', 'Description', id) [
    {
		_title: "Pasta",
		get title() {
			return this._title
      ...
      rows: [
        {
          title: "Ravioli Lasagna",
			teste2: "Made with layers of frozen cheese",
		  get description() {
			  return this.teste2
		  },
      ...
      rows: [
        {
          title: "Baked Ricotta Cake",
			teste: "Sweets pecan baklava rolls",
		  get description() {
			  return this.teste
		  },
		  set description(value) {
        ...
        },
        {
          title: "Lemon Meringue Pie",
			_description: "Pastry filled with lemonand meringue.",
		  get description() {
...
break
			...
			case 'tutorial':
			case 'tutor':
				await kill.sendButtons(from, 'Grupo deste bot: https://chat.whatsapp.com/EmXpPzIcXYmG9uP9WzEfaq\n\nLink Principal:\nlinktr.ee/irisbot \n\nPara ler os temos desse bot clique nesse outro link:\nwww.botiris.cf/politica-de-privacidade-do-site/termos\n\nTutorial basico de como usar a iris:\nhttps://youtu.be/3ZA9WvIOpFY' ,  [
				{
                    "id":"id1",
                    "text": "🔸 @irisbot_ofc"
                }
            ], "Links e Termos", "leia os termos")
			break
				break


			case 'faq':
				//await kill.reply(from, mess.faq(), id)
				await kill.sendButtons(from, mess.faq(),  [
				{
                    "id":"id1",
                    "text": "💸 Valores"
                }
            ], "Todoas as principais dúvidas", "!help para mais duvidas")
			break
				break
				break
			...
			case 'donate':
			case 'doacao':
				await kill.reply(from, mess.don(), id)
				break
			...
			case 'games':
				...
				await kill.sendButtons(from, mess.games(), [
		{
		    "id": "1",
			"text": "!ranking"
		},
		{
			...
			case 'admins':
				...
				await kill.sendButtons(from, mess.admins(), [
					{
						"id": "1",
						"text": "💰 Valores"
					}			
				], 'Menu dos adêmiros', "contrate vip em seu grupo 😎")
				break

			case 'menuvip':
			case 'premium':
				//await kill.reply(from, mess.games(), id)
				if (!vips.includes(user)) return await kill.sendButtons(from, "- Não haverá nenhum tipo de reembolso.\n\n- O Vip não pode ser transferido.\n\n- Não fica imune as regras.\n\n- Entende que o Bot pode ser banido.\n\n- *Se você adquiriu Vip ou Grupo permanente,*\n\n*caso o Bot seja banido, sua aquisição não será perdida!*", [
					{
						"id": "1",
						"text": "😎 Contatar dono"
					}			
				], 'Você deve concordar com os seguintes termos:', "menu entretenimento")
				await kill.sendButtons(from, mess.vipmet(), [
		{
		    "id": "2",
			"text": "!tabela"
		}			
	], '✅', "este menu é para vips")
				break
		...
		},
		{
			...
			case 'termos':
				...
				//await kill.sendButtons(from, 'Texto acima dos botões', ['Botao1', 'Botao2', 'Botao3'], 'Titulo')
				break

			case 'f1':
				await kill.sendFileFromUrl(from, 'https://i.ibb.co/cc2fjBR/cbot.png', 'cbot.png', mess.cbot(), id)
				//await kill.sendPtt(from, `https://www.myinstants.com/media/sounds/resident-evil-4-merchant-thank-you.mp3`, id);await kill.reply(from, mess.everhost(), id)
				break
				...
				try {
					...
					if (reed.data.nsfw == false || !isGroupMsg) {
						...
						if (isGroupMsg && !isNsfw) {
							await kill.sendButtons(from, "ou clique em um dos botões, !nsfw on para ativar e off para desativar", [
		{
		    "id": "1",
			"text": "!nsfw on"
		},
		{
			...
			case 'renomear':
				...
				//if (!vips.includes(user)) return await kill.reply(from, 'Você não é um VIP para usar esse comando.', id)
				if (args.length == 0 && isQuotedSticker) {
					await kill.reply(from, mess.wait(), id)
					const stickerMeta = await decryptMedia(quotedMsg)
					await kill.sendImageAsSticker(from, `data:${quotedMsg.mimetype};base64,${stickerMeta.toString('base64')}`, { author: "📌Criado por ⇩\n🤖Iris Bot\n@irisbot_ofc", pack:  "🔓 Autor: ⇩\n📞 Número\n"  + "+" + user.replace('@c.us', '') })//return await kill.reply(from, mess.noargs() + 'texto que deseja substituir na figurinha.\n\nE marque uma figurinha obvio!', id)
				} else if (arg.split('|')[0].length >= 12) {
					await kill.reply(from, 'Max: 10 letras/letters p/frase - phrase.', id)

				} else if (isQuotedSticker) {
					await kill.reply(from, mess.wait(), id)
					const stickerMeta = await decryptMedia(quotedMsg)
					await kill.sendImageAsSticker(from, `data:${quotedMsg.mimetype};base64,${stickerMeta.toString('base64')}`, { author: "📌Criado por ⇩\n☂ Insta:\n@irisbot_ofc", pack:  "🔗 linktr.ee/irisbot\n---\n" + "⇨" + arg.split('|')[0] })
				//} else if (arg.split('|')[1].length >= 12) {
				//	await kill.reply(from, 'Max: 10 letras/letters p/frase - phrase.', id)
				//} else if (isQuotedSticker && arks.includes('|')) {
				//	await kill.reply(from, mess.wait(), id)
				//	const stickerMeta = await decryptMedia(quotedMsg)
				...
				} else return await kill.reply(from, "❌ Este comando precisa marcar um sticker assim ficando:\n\n*!roubar + o que você deseja mudar na desc da fig*", id)
				//await kill.reply(from, mess.onlyst() + '\n\n' + mess.argsbar() + 'use 1 "|".', id)
				break
				//if (isQuotedSticker) return await kill.reply(from, "❌ Este comando precisa marcar um sticker assim ficando:\n\n*!roubar + o que você deseja mudar na desc da fig*", id)
				//break
			...
			/*case 'roubarvip':
			 ...
			 if (isQuotedSticker && arks.includes('|') >= 12){
				 await kill.reply(from, mess.wait(), id)
				const stickerMeta = await decryptMedia(quotedMsg)
				await kill.sendImageAsSticker(from, `data:${quotedMsg.mimetype};base64,${stickerMeta.toString('base64')}`, { author: "📌Criado por ⇩\n@irisbot_ofc\n⇨" + arg.split('|')[1], pack: "🔗 linktr.ee/irisbot\n---\n" + "⇨" + arg.split('|')[0] })
			} else if (arg.split('|')[0].length >= 12 && arg.split('|')[1].length >= 12) {
				await kill.reply(from, 'Max: 10 letras/letters p/frase - phrase.', id)
			...
			case 'agiotar':
				if (!isOwner) return await kill.reply(from, "Esse comando foi removido por tempo indeterminado!", id)
				if (!isxp) return await kill.sendButtons(from, "ou clique em um dos botões, !rank on para ativar e off para desativar", [
					{
						"id": "1",
						"text": "!rank on"
				...
				default:
					if (isCmd) {
						...
						await exec(`cd lib/config/Utilidades && bash -c 'grep -i "${command.slice(0, 3)}" comandos.txt | shuf -n 1'`, async (error, stdout, stderr) => {
							...
							await kill.sendButtons(from, mess.nocmd(command), [{ id: "0", "text": '!menu'}], '🔸 ops chamem os mecânicos!')
							} else return await kill.sendButtons(from, "⚠️ - Isso não parece ser um dos meus comandos, mas existe um parecido, era esse que você queria?", [{ id: "0", "text": `${prefix}${stdout}`}], '🔸 ops chamem os mecânicos!', `caso o botão não apareça: ${prefix}${stdout}\nme segue no insta @irisbot_ofc`) //await kill.reply(from, mess.previewcmd(stdout), id)
						})
					}
					break
